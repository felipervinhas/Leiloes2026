...
export async function criarLeilao(d: Omit<Leilao, 'id' | 'nomeCidade' | 'nomeEstado' | 'descricaoCondicao'>): Promise<number> {
  ...
  const r = await pool.request()
    ...
    .input('comven', sql.Decimal(18, 2), d.comven||null).input('comcom', sql.Decimal(18, 2), d.comcom||null)
...
export async function atualizarLeilao(id: number, d: Omit<Leilao, 'id' | 'nomeCidade' | 'nomeEstado' | 'descricaoCondicao'>): Promise<void> {
  ...
  await pool.request()
    ...
    .input('condic', sql.Int, d.condic||null).input('comven', sql.Decimal(18, 2), d.comven||null)
    .input('comcom', sql.Decimal(18, 2), d.comcom||null).input('ativox', sql.VarChar, d.ativox||'S')
