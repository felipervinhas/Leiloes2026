...
async function executarMigracaoBanco(banco: string) {
  try {
    ...
    const queries = [
      ...
      `IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'CLIENTES_PROPRIEDADES' AND COLUMN_NAME = 'CEP')
       ALTER TABLE CLIENTES_PROPRIEDADES ADD CEP VARCHAR(9) NULL`,
      `IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'CLIENTES_PROPRIEDADES' AND COLUMN_NAME = 'INCRA')
       ALTER TABLE CLIENTES_PROPRIEDADES ADD INCRA VARCHAR(20) NULL`,
    ];
