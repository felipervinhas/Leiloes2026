...
const TIPO_INFO: Record<string, { titulo: string; frase: (cliente: string) => string }> = {
  D: { titulo: 'RECIBO DE DESPESA', frase: c => `Pagamos a ${c || 'acima identificado(a)'} a quantia de` },
  S: { titulo: 'RECIBO DE DESPESA', frase: c => `Pagamos a ${c || 'acima identificado(a)'} a quantia de` },
  C: { titulo: 'RECIBO DE CRÉDITO', frase: c => `Recebemos de ${c || 'acima identificado(a)'} a quantia de` },
  E: { titulo: 'RECIBO DE CRÉDITO', frase: c => `Recebemos de ${c || 'acima identificado(a)'} a quantia de` },
  F: { titulo: 'RECIBO DE PAGAMENTO', frase: c => `Pagamos a ${c || 'acima identificado(a)'} a quantia de` },
};
  ...
  return (
    <Document title={`${info.titulo} #${dados.id}`} author={nomeEmpresa}>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.frase}>{info.frase(dados.cliente || '')}</Text>
        <Text style={s.extenso}>{valorExtenso(dados.valor)}.</Text>

        <View style={s.detalhesBox}>
          <View style={s.linha}>
            <Text style={s.label}>Cliente</Text>
            <Text style={s.valor}>{dados.cliente || '—'}</Text>
          </View>
          <View style={s.linha}>
            <Text style={s.label}>Leilão</Text>
            <Text style={s.valor}>{dados.leilao || '—'}</Text>
          </View>
          <View style={s.linha}>
        ...
        <View style={s.assinatura}>
          ...
          <Text style={s.assinaturaNome}>{dados.cliente || nomeEmpresa}</Text>
...
export default RelatorioReciboDespesa;
