...
/**
 * Cobre o cadastro de Classificações de ponta a ponta: cria, confirma
 * persistência ao reabrir para edição, edita e por fim exclui o próprio
 * registro criado via API.
 */
...
test('cadastro de classificações: criar, confirmar persistência, editar e excluir', async ({
  page,
  request,
}) => {
  const sufixo = Date.now();
  const descricaoUnica = `E2E CLASSIFICACAO ${sufixo}`;
  const descricaoEditada = `${descricaoUnica} EDITADA`;

  await loginUI(page);
  await page.goto(`/${BANCO}/classificacoes`);

  // ── Criar ────────────────────────────────────────────────────────────
  await page.click('button:has-text("Nova Classificação")', { force: true });
  await page.waitForTimeout(500);
  await expect(page.locator('text=Nova Classificação').first()).toBeVisible();
  ...
  await page.fill('input[placeholder="Buscar por descrição..."]', descricaoUnica);
  ...
  await page.fill('input[placeholder="Buscar por descrição..."]', descricaoEditada);
  await page.keyboard.press('Enter');
  await page.waitForTimeout(1000);
  await expect(page.locator('table tbody tr', { hasText: descricaoEditada })).toHaveCount(1);

  // ── Limpeza: exclui via API a classificação criada neste teste ─────
  const token = await login(request);
  const buscaRes = await request.get(`${API_URL}/api/${BANCO}/classificacoes`, {
    headers: { Authorization: `Bearer ${token}` },
    params: { busca: descricaoEditada },
  });
  const encontrados = await buscaRes.json();
  expect(encontrados.length, 'deveria achar a classificação de teste para limpar').toBeGreaterThan(0);
  for (const c of encontrados) {
    await request.delete(`${API_URL}/api/${BANCO}/classificacoes/${c.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
});
